"""
True out-of-sample test on 2025 data ONLY.
Train on ALL historical data (2000-2024), test on most recent 2025 data.
This verifies there's no data leakage and model works on truly unseen recent data.
"""
...
print("2025-ONLY OUT-OF-SAMPLE TEST")
print("="*80)
...
# Split at 2025 - train on ALL historical data, test on 2025 only
...
if len(test_data) == 0:
    print("\nERROR: No 2025 data available!")
    print("Latest date in dataset:", df_clean.index.max())
    exit(1)
...
print(f"\n{'='*80}")
print("2025-ONLY OUT-OF-SAMPLE BACKTEST RESULTS")
print(f"{'='*80}")
print(f"\nCapital:")
...
print(f"  2025-only test (train 2000-2024): {final_return*100:+.2f}% total, {annualized_return*100:.2f}% annualized")

if final_return > 0:
    print(f"\n  SUCCESS! Model works on most recent 2025 data - NO DATA LEAKAGE!")
else:
    print(f"\n  Model performance on 2025: {final_return*100:.2f}%")

print(f"{'='*80}")

# Analyze predictions vs actuals
print(f"\nPrediction Quality on 2025 Data:")
print(f"  Predicted returns - Q1: {np.percentile(y_pred, 25):.6f}")
print(f"  Predicted returns - Median: {np.median(y_pred):.6f}")
print(f"  Predicted returns - Q3: {np.percentile(y_pred, 75):.6f}")
print(f"\n  Actual returns - Mean: {np.mean(y_test):.6f}")
print(f"  Actual returns - Std: {np.std(y_test):.6f}")

# Check actual returns by signal
if n_buy > 0:
    long_actuals = y_test[signals == 1]
    print(f"\n  Actual returns when model said LONG: {np.mean(long_actuals):.6f}")
if n_sell > 0:
    short_actuals = y_test[signals == -1]
    print(f"  Actual returns when model said SHORT: {np.mean(short_actuals):.6f}")
if n_hold > 0:
    hold_actuals = y_test[signals == 0]
    print(f"  Actual returns when model said HOLD: {np.mean(hold_actuals):.6f}")
...
with open('test_2025_only_results.pkl', 'wb') as f:
    pickle.dump(results, f)

print(f"\nResults saved to: test_2025_only_results.pkl")
print(f"{'='*80}")
