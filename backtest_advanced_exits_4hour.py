"""
Advanced exit strategies for 4-HOUR forex trading.

Adapted from daily strategy with key changes:
1. Bar-based hold periods (30 bars = 5 days equivalent)
2. Scaled stop/target levels for 4-hour volatility
3. Bar-based cooldown (6 bars = 1 day)

Usage:
    python backtest_advanced_exits_4hour.py --pair EURUSD
    python backtest_advanced_exits_4hour.py --pair GBPUSD --target target_1day_return
"""
...
print("="*80)
print(f"4-HOUR ADVANCED EXIT STRATEGIES BACKTEST - {CURRENCY_PAIR}")
print("="*80)
...
print(f"  Train:      {TRAIN_BARS} bars (~{TRAIN_BARS/6:.0f} days)")
print(f"  Val:        {VAL_BARS} bars (~{VAL_BARS/6:.0f} days)")
print(f"  Test:       {TEST_BARS} bars (~{TEST_BARS/6:.0f} days)")
print(f"  Roll:       {ROLL_BARS} bars (~{ROLL_BARS/6:.0f} days)")
...
def backtest_advanced_exits_4hour(signals, df_prices, test_indices,
    ...
    - max_hold_bars: Maximum bars to hold (30 bars = 5 days)
    - loss_cooldown_bars: Bars to wait after loss (6 bars = 1 day)
...
# Test configurations adapted for 4-hour trading
test_configs = [
    # Baseline - tighter stops for 4-hour
    {
        'name': 'Baseline (0.30% SL / 0.75% TP, 30 bar hold)',
        'base_stop_loss_pct': 0.0030,
        'base_take_profit_pct': 0.0075,
        'use_vol_adjustment': False,
        'max_hold_bars': 30,
    ...
    {
        'name': 'Baseline (0.40% SL / 1.00% TP, 30 bar hold)',
    ...
    # Vol-adjusted only
    {
        'name': 'Vol-adj (base 0.30% / 0.75%, 30 bar hold)',
        'base_stop_loss_pct': 0.0030,
        'base_take_profit_pct': 0.0075,
    ...
    {
        'name': 'Vol-adj (base 0.25% / 0.60%, 30 bar hold)',
    ...
    {
        'name': 'Vol-adj (base 0.40% / 1.00%, 30 bar hold)',
        'base_stop_loss_pct': 0.0040,
        'base_take_profit_pct': 0.0100,
        'use_vol_adjustment': True,
        'vol_window': 20,
    ...
    # Vol-adjusted + Loss Cooldown (6 bars = 1 day)
    {
        'name': 'Vol-adj (0.30%/0.75%) + 6 bar cooldown',
        'base_stop_loss_pct': 0.0030,
        'base_take_profit_pct': 0.0075,
        'use_vol_adjustment': True,
        'vol_window': 20,
    ...
    {
        'name': 'Vol-adj (0.30%/0.75%) + 12 bar cooldown',
        'base_stop_loss_pct': 0.0030,
        'base_take_profit_pct': 0.0075,
        'use_vol_adjustment': True,
        'vol_window': 20,
    ...
    {
        'name': 'Vol-adj (0.30%/0.75%) + 18 bar cooldown',
        'base_stop_loss_pct': 0.0030,
        'base_take_profit_pct': 0.0075,
        'use_vol_adjustment': True,
        'vol_window': 20,
    ...
    {
        'name': 'Vol-adj (0.30%/0.75%, 24 bar hold) + 6 bar cooldown',
        'base_stop_loss_pct': 0.0030,
        'base_take_profit_pct': 0.0075,
        'use_vol_adjustment': True,
        'vol_window': 20,
    ...
    {
        'name': 'Vol-adj (0.30%/0.75%, 18 bar hold) + 6 bar cooldown',
        'base_stop_loss_pct': 0.0030,
        'base_take_profit_pct': 0.0075,
        'use_vol_adjustment': True,
        'vol_window': 20,
...
print("\n" + "="*80)
print("TESTING 4-HOUR EXIT STRATEGIES")
print("="*80)
...
# Save results
if MODEL_SUFFIX:
    output_file = f'advanced_exits_4hour_{CURRENCY_PAIR}_{TARGET_COLUMN}_{MODEL_SUFFIX}_results.json'
else:
    output_file = f'advanced_exits_4hour_{CURRENCY_PAIR}_{TARGET_COLUMN}_results.json'
