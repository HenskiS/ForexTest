#!/bin/bash
#
# Setup 10-Asset Trading Bot on Server (After Git Pull)
#
# This script configures the server to run the 10-asset trading bot after you've
# already done a git pull to update the code.
#
# Usage: ./setup_10_asset_server.sh
#

set -e  # Exit on error

echo "=================================================="
echo "SETTING UP 10-ASSET TRADING BOT"
echo "=================================================="
echo ""
echo "This will:"
...
cat > run_10_asset_trading.sh << 'EOF'
#!/bin/bash
# Daily 10-asset trading bot execution script
...
echo "====================================" >> $LOG_FILE
echo "10-Asset Trading Bot: $(date)" >> $LOG_FILE
echo "====================================" >> $LOG_FILE

# Run the 10-asset trader (use --yes to skip confirmations)
python3 oanda_10_asset_trader.py --live --yes >> $LOG_FILE 2>&1
...
chmod +x run_10_asset_trading.sh
...
(crontab -l 2>/dev/null; echo "0 14 * * * $SCRIPT_DIR/run_10_asset_trading.sh") | crontab -

echo "✓ Cron job installed:"
crontab -l | grep "run_10_asset_trading.sh"

echo ""
echo "Step 4: Running dry-run test..."
source venv/bin/activate
python3 oanda_10_asset_trader.py --live --dry-run --yes
...
echo ""
echo "Configuration:"
echo "  - Trading bot: oanda_10_asset_trader.py"
echo "  - Schedule: Daily at 9 AM EST (14:00 UTC)"
echo "  - Mode: LIVE trading (real money)"
echo "  - Assets: 10 (4 Forex + 5 Metals + 5 Commodities/Indices)"
echo "  - Leverage: 2x ($100 per asset on $500 account)"
echo "  - Expected return: ~168% annual"
...
echo "  - Manual dry-run: ./venv/bin/python3 oanda_10_asset_trader.py --live --dry-run"
echo "  - Manual live run: ./venv/bin/python3 oanda_10_asset_trader.py --live --yes"
echo ""
