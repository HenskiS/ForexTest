"""
Analyze annual performance for ROLLING DAILY retraining approach
...
print("ROLLING DAILY RETRAINING - ANNUAL PERFORMANCE ANALYSIS")
print("="*80)
...
# Create combined results format
combined_results = {
...
# Convert to format compatible with existing backtest code
all_dates = pd.to_datetime(combined_results['dates'])
all_predictions = np.array(combined_results['predictions'])
all_actuals = np.array(combined_results['actuals'])

print(f"Loaded {len(all_predictions)} days of rolling daily predictions")
...
def generate_signals_optimized(predictions, lower_pct=48, upper_pct=52):
    """Generate signals using optimized thresholds (48th/52nd percentile for EURUSD)."""
...
# Generate signals for all predictions
signals = generate_signals_optimized(all_predictions, lower_pct=48, upper_pct=52)
...
print(f"Matched {len(test_indices)} prediction dates to price data")

# Run backtest on all predictions
all_trades = backtest_with_dates(
    all_actuals,
    all_predictions,
    signals,
    df_clean,
