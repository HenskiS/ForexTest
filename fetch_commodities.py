"""
Fetch 9 AM EST aligned historical data for commodity pairs.
Fetches XAU/USD (Gold), XAG/USD (Silver) for expansion testing.
"""
import sys
import os
import io
...
def main():
    print("="*80)
    print("FETCHING COMMODITY PAIRS FOR BACKTEST")
    print("="*80)
    print(f"\nPairs to fetch: {', '.join(COMMODITY_PAIRS)}")
    ...
    if successful == total:
        ...
        print("\nNext steps:")
        print("  python backtest_oanda_data.py --pair XAUUSD --test-days 750 --spread-pct 0.015")
        print("  python backtest_oanda_data.py --pair XAGUSD --test-days 750 --spread-pct 0.015")
    else:
