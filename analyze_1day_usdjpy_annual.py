"""
Analyze annual performance for 1-day USDJPY model with optimized thresholds
...
print("1-DAY USDJPY ANNUAL PERFORMANCE ANALYSIS")
print("="*80)

# Load data
df = pd.read_csv('data/USDJPY_1day_with_features_FIXED_multitarget.csv',
...
# Load predictions
with open('xgboost_results_USDJPY_target_5day_return.pkl', 'rb') as f:
    all_results = pickle.load(f)

print(f"Loaded {len(all_results)} windows of predictions")

def generate_signals_optimized(predictions, lower_pct=35, upper_pct=65):
    """Generate signals using optimized thresholds (35th/65th percentile for USDJPY)."""
...
print("\nBacktesting with OPTIMIZED thresholds (35th/65th percentile)...")
print("Configuration:")
...
for window_idx, window_results in enumerate(all_results):
    ...
    # Use OPTIMIZED thresholds (48/52 for USDJPY)
    signals = generate_signals_optimized(predictions, lower_pct=35, upper_pct=65)
