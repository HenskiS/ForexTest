...
def run_netting_dca_stop(max_slots=5, full_size_per_trade=True, close_on_opposite=False, tighten_sl=None, check_entry_day_stop=True):
    """
...
if __name__ == '__main__':
    print('=' * 85)
    print('8-PAIR SLEEP WELL BACKTEST - BUG FIX COMPARISON')
    print('=' * 85)
    print()
    print('Config: 8 pairs, 22.5% alloc/slot, 1.5x leverage, 2% SL, 5-day hold')
    print()

    # Run BUGGY version (no entry-day stop check) for comparison
    print('Running OLD (buggy) version - no entry-day stop check...')
    trades_old, pnl_old = run_netting_dca_stop(max_slots=5, full_size_per_trade=True, check_entry_day_stop=False)
    m_old = calc_metrics(trades_old, pnl_old)

    # Run FIXED version
    print('Running FIXED version - with entry-day stop check...')
    trades, pnl = run_netting_dca_stop(max_slots=5, full_size_per_trade=True, check_entry_day_stop=True)
    m = calc_metrics(trades, pnl)
    yearly = calc_yearly_metrics(trades, pnl)

    print()
    print('=' * 85)
    print('COMPARISON: OLD (BUGGY) vs FIXED')
    print('=' * 85)
    print(f'{"Metric":<15} {"OLD (buggy)":>15} {"FIXED":>15} {"Difference":>15}')
    print('-' * 60)
    print(f'{"Trades":<15} {m_old["trades"]:>15,} {m["trades"]:>15,} {m["trades"] - m_old["trades"]:>+15,}')
    print(f'{"Win Rate":<15} {m_old["win_rate"]:>14.1f}% {m["win_rate"]:>14.1f}% {m["win_rate"] - m_old["win_rate"]:>+14.1f}%')
    print(f'{"Annual Return":<15} {m_old["annual"]:>14.1f}% {m["annual"]:>14.1f}% {m["annual"] - m_old["annual"]:>+14.1f}%')
    print(f'{"Sharpe":<15} {m_old["sharpe"]:>15.2f} {m["sharpe"]:>15.2f} {m["sharpe"] - m_old["sharpe"]:>+15.2f}')
    print(f'{"Max DD":<15} {m_old["max_dd"]:>14.1f}% {m["max_dd"]:>14.1f}% {m["max_dd"] - m_old["max_dd"]:>+14.1f}%')

    print()
    print('=' * 85)
    print('FIXED VERSION - YEARLY PERFORMANCE')
    print('=' * 85)
    print(f'{"Year":<6} {"Return":>10} {"MaxDD":>10} {"WinRate":>10} {"Trades":>8} {"SL":>6} {"SL_Day0":>8}')
    print('-' * 85)
    ...
    for year in sorted(yearly.keys()):
        y = yearly[year]
        total_sl += y['sl_exits']
        total_sl_day0 += y['sl_day0']
        print(f'{year:<6} {y["return"]:>9.1f}% {y["max_dd"]:>9.1f}% {y["win_rate"]:>9.1f}% {y["trades"]:>8} {y["sl_exits"]:>6} {y["sl_day0"]:>8}')

    print('-' * 85)
    print(f'TOTAL STOPS: {total_sl + total_sl_day0} (Later days: {total_sl}, Entry day: {total_sl_day0})')
    print()
    print('NOTE: SL_Day0 = stops hit on entry day (previously missed by buggy code)')
