...
XGBoost model training, prediction, and signal generation.
"""
import os
import pickle
import numpy as np
...
class TradingModel:
    """XGBoost trading model with rolling window training"""
    ...
    def __init__(self, pair):
        ...
        self.buffer_file = f'data/oanda_cache/prediction_buffer_{self.pair}.pkl'
    ...
    def train(self, df):
        ...
        """
        print(f"\n{'='*70}")
        print(f"Training model on last {TradingConfig.TRAIN_WINDOW_SIZE} days...")
        print(f"{'='*70}")
    ...
    def generate_signal(self, prediction, lower_percentile=None, upper_percentile=None):
        ...
        Args:
            ...
            lower_percentile: Lower percentile threshold (default from config)
            upper_percentile: Upper percentile threshold (default from config)
        ...
        if prediction >= upper_threshold:
            signal = 1  # Long
            print(f"  Signal: LONG (prediction >= upper threshold)")
        elif prediction <= lower_threshold:
            signal = -1  # Short
            print(f"  Signal: SHORT (prediction <= lower threshold)")
        else:
            signal = 0  # No signal
