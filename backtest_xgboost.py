"""
Backtest XGBoost predictions using same forex mechanics as LSTM.
"""
...
print("Loading data and XGBoost results...")
df = pd.read_csv('data/EURUSD_1day_with_features_FIXED.csv', index_col='date', parse_dates=True)
df_clean = df.dropna()

with open('xgboost_results_FIXED.pkl', 'rb') as f:
    all_results = pickle.load(f)

print(f"Loaded {len(all_results)} windows of XGBoost predictions")
...
# Save results
with open('xgboost_performance_summary.json', 'w') as f:
    json.dump(performance_summary, f, indent=2)

for strategy, results in results_by_strategy.items():
    with open(f'xgboost_results_{strategy}.pkl', 'wb') as f:
        pickle.dump(results, f)

print(f"\n{'='*80}")
print("Results saved:")
print("  xgboost_performance_summary.json")
for strategy in strategies:
    print(f"  xgboost_results_{strategy}.pkl")
print(f"{'='*80}")
