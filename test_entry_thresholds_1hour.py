"""
Test various entry threshold levels for 1-hour ML strategy.

Test different threshold levels to find optimal signal selectivity for 1-hour timeframe.

Usage:
    python test_entry_thresholds_1hour.py --pair EURUSD
"""
...
print("="*80)
print(f"TESTING ENTRY THRESHOLDS - {CURRENCY_PAIR} 1-HOUR")
print("="*80)
...
# Walk-forward configuration (BAR-BASED for 1-hour)
TRAIN_BARS = 7200  # ~300 days
VAL_BARS = 1872    # ~78 days
TEST_BARS = 1512   # ~63 days
ROLL_BARS = 1512   # Roll forward by test period
...
def backtest_advanced_exits_1hour(signals, df_prices, test_indices,
                                   ...
                                   vol_window=20,
                                   max_hold_bars=120,  # 5 days for 1-hour
                                   loss_cooldown_bars=24,  # 1 day for 1-hour
...
def backtest_threshold_config(results_df, df_clean, windows,
    ...
    # Use same exit config as 4-hour (scaled to 1-hour bars)
    exit_config = {
        'base_stop_loss_pct': 0.0030,
        'base_take_profit_pct': 0.0075,
        'use_vol_adjustment': True,
...
def calculate_performance_metrics(backtests):
    ...
    # Annualize returns (1-hour bars: ~24 bars per day, ~252 trading days)
...
# Test different threshold levels (focusing on range that worked for 4-hour)
threshold_configs = [
    (10, 90, "Top/Bottom 10%"),
    (20, 80, "Top/Bottom 20%"),
...
print("="*80)
print(f"Note: Only {len(windows)} windows trained (not full 87 windows)")
