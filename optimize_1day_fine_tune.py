"""
Fine-tune optimization for 1-day holding period model.

Now that we know 1-day holding is optimal, let's do a finer-grained search
on stop loss and take profit levels to find the absolute best configuration.
"""
import pandas as pd
import numpy as np
import pickle
...
print(f"Fine-Tuning 1-Day Model Parameters - {PAIR}")
print("="*80)
print(f"Test period: Last {TEST_DAYS} days")
print(f"Fixed: 1-day holding period")
print("="*80)
...
# Fine-grained parameter grid for 1-day holding period
param_grid = {
    'base_stop_loss_pct': [
        0.0015, 0.0018, 0.0020, 0.0022, 0.0025,
        0.0028, 0.0030, 0.0035, 0.0040, 0.0045, 0.0050
    ],
    'base_take_profit_pct': [
        0.0030, 0.0035, 0.0040, 0.0045, 0.0050,
        0.0060, 0.0070, 0.0080, 0.0090, 0.0100,
        0.0110, 0.0120, 0.0130, 0.0150, 0.0180, 0.0200
    ]
}

print("\n" + "="*80)
print("FINE-GRAINED PARAMETER OPTIMIZATION (1-DAY HOLDING)")
print("="*80)
print(f"Testing {len(param_grid['base_stop_loss_pct']) * len(param_grid['base_take_profit_pct'])} combinations...")
print()

def backtest_strategy(predictions, df_prices, test_indices,
                      lower_pct=48, upper_pct=52,
                      base_stop_loss_pct=0.0020,
...
for stop_loss, take_profit in tqdm(combinations, desc="Testing combinations"):
    result = backtest_strategy(
        ...
        loss_cooldown_days=1,
        transaction_cost_pct=0.0002,
        holding_period=1,  # Fixed at 1 day
        buffer_warmup=50
    )
...
print("\n" + "="*80)
print(f"TOP 30 PARAMETER COMBINATIONS - {PAIR} (1-DAY HOLDING)")
print("="*80)
...
print(f"  Stop Loss: {best_return['stop_loss_pct']:.3f}%")
print(f"  Take Profit: {best_return['take_profit_pct']:.3f}%")
...
print(f"  Avg Win: {best_return['avg_win']:.2f}%")
print(f"  Avg Loss: {best_return['avg_loss']:.2f}%")

best_sharpe = results_df.sort_values('sharpe', ascending=False).iloc[0]
print(f"\nBest Sharpe Ratio: {best_sharpe['sharpe']:.2f}")
print(f"  Stop Loss: {best_sharpe['stop_loss_pct']:.3f}%")
print(f"  Take Profit: {best_sharpe['take_profit_pct']:.3f}%")
...
print(f"  Stop Loss: {best_winrate['stop_loss_pct']:.3f}%")
print(f"  Take Profit: {best_winrate['take_profit_pct']:.3f}%")
print(f"  Total Return: {best_winrate['total_return']:.2f}%")
print(f"  Sharpe: {best_winrate['sharpe']:.2f}")

best_profit_factor = results_df.sort_values('profit_factor', ascending=False).iloc[0]
print(f"\nBest Profit Factor: {best_profit_factor['profit_factor']:.2f}")
print(f"  Stop Loss: {best_profit_factor['stop_loss_pct']:.3f}%")
print(f"  Take Profit: {best_profit_factor['take_profit_pct']:.3f}%")
print(f"  Total Return: {best_profit_factor['total_return']:.2f}%")
print(f"  Win Rate: {best_profit_factor['win_rate']:.1f}%")

# Save results
output_file = f'{PAIR}_1day_fine_tuned_{TEST_DAYS}days.csv'
